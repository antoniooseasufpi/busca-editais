...
from radar_agro.classifiers.llm_classifier import (
    classify_opportunity,
    classify_prefiltered_opportunity,
    should_prefilter_by_publication_date,
)
...
def run_search_pipeline(progress_callback: Callable[[dict], None] | None = None) -> dict:
    ...
    for index, item in enumerate(raw_results, start=1):
        ...
        if should_prefilter_by_publication_date(item):
            _emit(
                progress_callback,
                "classificacao",
                f"Pré-filtro de data {index}/{total_results}: {title}",
                current=index,
                total=total_results,
                stage_percent=stage_percent,
                overall_percent=40 + int(stage_percent * 0.55),
            )
            classified.append(classify_prefiltered_opportunity(item))
            prefiltered_count += 1
            continue
        ...
        _emit(
            progress_callback,
            "classificacao",
            f"Classificando {index}/{total_results}: {title}",
            current=index,
            total=total_results,
            stage_percent=stage_percent,
            overall_percent=40 + int(stage_percent * 0.55),
